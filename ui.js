// ui.js - ML-style controls: joystick + skill pad + cooldown overlays + lock toggle
export class InputMux {
  ...
  _setupMobile(){
    ...
    stick.addEventListener('touchmove', ev=>{
      ...
      const dx = p.x - base.x, dy = p.y - base.y; const max=56;
    ...
    const bindDrag = (el, type)=>{
      ...
      const onEnd = ev=>{
        if (this.drag && this.drag.type===type){ this.cast = type; }
    ...
    const positionAim = (sx, sy, dx, dy)=>{
      const length = Math.min(180, Math.hypot(dx, dy));
      const angle = Math.atan2(dy, dx);
      aimLine.style.left = sx+'px'; aimLine.style.top = sy+'px';
      aimLine.style.width = length+'px';
      aimLine.style.transform = `translateX(10px) rotate(${angle}rad)`;
  ...
  setCooldowns(rem){ // {A:seconds, Q:seconds, E:seconds}
    ...
    cdA.textContent = fmt(rem.A||0);
    cdQ.textContent = fmt(rem.Q||0);
    cdE.textContent = fmt(rem.E||0);
    // Gray out skills when on cooldown
    document.getElementById('btnAttack').classList.toggle('locked', (rem.A||0)>0);
    document.getElementById('btnQ').classList.toggle('locked', (rem.Q||0)>0);
    document.getElementById('btnE').classList.toggle('locked', (rem.E||0)>0);
  }
